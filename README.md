...
### 7-2. 커버링 인덱스를 활용한 좌석 조회 성능 최적화
* **목적**: 좌석 데이터 조회 효율화.
* **방법**: 좌석 조회에 필요한 컬럼(공연 회차, 좌석 ID, 좌석 상태)을 포함하는 커버링 인덱스를 적용하여 필요한 데이터를 인덱스에서 조회하도록 개선함.
* **이점**:
    - **좌석 데이터 조회 성능 향상**: 좌석 데이터 30,000건 기준 쿼리 실행 시간을 40ms → 13ms로 약 66% 개선함.
    - **조회 비용 감소**: 조회에 필요한 데이터를 인덱스에서 처리하도록 개선하여 테이블 접근 비용을 줄임.
